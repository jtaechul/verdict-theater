...
## ⚠️ 해상도는 프롬프트로 못 정한다 (2026-08-12 실측)

아래 블록에는 `at least 2048 x 4096 pixels` 가 **7개 전부** 적혀 있다.
그런데 실제로 나온 시트 7장은 예외 없이 **1.05~1.08 MP** 였다 —
비율만 다르고 픽셀 총량은 똑같았다. 모델은 프롬프트의 크기 지시를 무시한다.

**크기는 API 로 정한다.** `src/assets_gen.py` 의 `gen_image` 가
`imageConfig: {aspectRatio, imageSize}` 를 보낸다(기본 `9:16` · `4K`).
환경변수 `GEMINI_IMAGE_SIZE` · `GEMINI_IMAGE_RATIO` 로 바꿀 수 있다.

그러니 블록 안의 해상도 문구는 **그대로 둬도 무해하지만 그것이 크기를 정하지는
않는다.** 크기가 아쉬우면 프롬프트가 아니라 위 환경변수를 손대야 한다.

### 비율은 아무 값이나 못 넣는다 (2026-08-13 실측)

3열 6행이니 `1:2` 가 맞다고 넣었다가 **HTTP 400 으로 전부 거절당했다.**
구글이 받아 주는 값은 다음뿐이다(400 응답 본문에 그대로 적혀 온다).

```
1:1  1:4  1:8  2:3  3:2  3:4  4:1  4:3  4:5  5:4  8:1  9:16  16:9  21:9
```

`1:2` 는 없다. 그래서 `sheet_ratio()` 가 `COLS/ROWS` 를 계산해 **받아 주는 값
가운데 가장 가까운 것**을 고른다 — 3열 6행이면 `9:16`(0.5625)이다.
(시킨 대로 3열 6행으로 그려진 유일한 시트 F50A 가 720x1456 = 0.495 였다.)

### 왜 4K 인가

한 장을 18칸으로 쪼개 쓰기 때문에, 한 장이 작으면 칸이 잘게 쪼개진다.

| 시트 한 장 | 칸 하나 | 전신 인물 | 화면(800px)에서 |
|---|---|---|---|
| 1MP (지금 있는 7장) | 약 228x224 | 105x302 | 2.6배 늘림 → 흐림 |
| 2K | 약 512x448 | 약 400px | 2배 늘림 |
| **4K** | 약 1024x896 | **약 800px** | **안 늘림** |

배경은 통째로 한 장을 쓰고 게다가 흐리게 깔리므로 `2K`(`IMAGE_SIZE_BG`)면 된다.

### 그림 만들기가 아예 막혀 있을 수 있다 (2026-08-13 실측)

그림 모델 셋(`gemini-3.1-flash-image` · `gemini-2.5-flash-image` ·
`gemini-3-pro-image`)이 전부 **무료로는 하루 0장**인 상태가 실제로 있었다.
글자(대본) 만들기는 같은 열쇠로 멀쩡히 되기 때문에 열쇠 문제로 보이지 않는다.

- 확인: 에셋 만들기 → **[그림 만들기 되는지 확인]** (막혀 있으면 0원)
- 풀기: [구글 AI 스튜디오 열쇠 화면](https://aistudio.google.com/apikey) 에서
  그 열쇠의 프로젝트에 결제를 걸어 둔다.
- `limit: 0` 은 **기다려도 안 풀린다.** 분당 제한과는 다른 것이다.
