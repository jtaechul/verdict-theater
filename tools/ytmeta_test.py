...
#    스스로 알아본다. 제목의 #shorts 는 하는 일 없이 제목 100자 중 8자를 먹고,
#    쇼츠 화면에서 제목이 잘리는 자리를 잡아먹는다. **붙으면 안 된다.**
# ⚠️ 2026-09-09 — 여기는 **16화 롱폼** 길이다(제목에 "(1화)" 가 붙는다).
#    같은 날 #shorts 를 되살린 것은 90초 쇼츠(part_meta) 쪽 이야기다.
#    한 번 여기까지 같이 뒤집었다가 빨간불이 났다 — 길이 다르면 규칙도 다르다.
ck("제목에 #shorts 가 안 붙는다", "#shorts" not in m1["title"].lower(),
   m1["title"])
...
ck("채널 태그가 늘 붙는다", "판결극장" in m1["tags"])
# ⭐ 2026-09-06 — 유튜브가 제목 위에 띄우는 것은 **앞의 세 개**뿐이다.
#    그래서 실측 검색량 1위(사연 666,590)를 맨 앞에 둔다.
ck("맨 앞이 '사연' 이다 (제목 위에 뜨는 자리)", m1["tags"][0] == "사연",
   " ".join(m1["tags"][:3]))
ck("검색량이 안 잡히던 태그는 뺐다",
   not ({"법률사연", "쇼츠드라마", "실화사연", "외도", "shorts"}
        & set(m1["tags"])), " ".join(m1["tags"]))
...
# ⚠️ 맨 앞에 '사연' 을 끼우므로 그 뒤부터 손님이 정한 태그다
ck("정해 준 태그를 쓴다", m["tags"][:4] == ["사연", "직접", "고른", "태그"],
   " ".join(m["tags"][:4]))
