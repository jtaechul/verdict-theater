...
# ⚠️⚠️ 2026-08-13 — 여기에 빼 달라고 하신 소리 넷이 **그대로 남아 있었다**
#    (clock·footsteps·heartbeat·phone). 파일을 지우고, 만드는 법을 지우고,
#    낱말표에서 빼고, 금지 목록에 넣었는데 **받아오는 법만 안 지웠다.**
#    [효과음 받아오기] 버튼 한 번이면 전부 되살아나는 상태였다.
#    손님: "왜 자꾸 이런 일이 반복되는 거야?" — 길을 하나씩만 막았기 때문이다.
#    ⭐ 이제 tools/sfx_test.py [5-2] 가 **소리가 생기는 길 다섯 곳을 전부** 훑는다.
#       여기에 금지된 이름을 넣으면 그 검사가 실패한다.
PROFILES = {
    # ⚠️⚠️ 2026-08-13 — 여기 "clock" 이 남아 있었다. **또 같은 실수다.**
    #    소리 파일을 지우고, 만드는 법(SFX_RECIPE)을 지우고, 낱말표에서 빼고,
    #    금지 목록에 넣었는데 — **받아오는 법을 안 지웠다.**
    #    [효과음 받아오기] 버튼 한 번이면 진짜 시계 초침이 다시 내려온다.
    #    손님: "시계 효과음 척척척척척은 아직 안 지워졌어."
    #    ⭐ 손님은 소리 품질이 아니라 **시계 초침 자체가 겉돈다**고 하셨다.
    #       진짜 녹음이어도 안 쓴다. 되살릴 길을 전부 막는다.
    #       (tools/sfx_test.py [5-2] 가 이제 이 표까지 같이 본다)
