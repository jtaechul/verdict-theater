...
def main_dir(d):
    ...
    if not d.is_dir():
        ...
        return 0
    # ⭐⭐⭐ 2026-09-13 — **한 그림에 한 번만 지운다.**
    #    까닭(값이 새던 자리): 이 함수는 stills 를 돌릴 때마다 무조건 돌면서
    #    **모든 그림 파일을 새로 저장**했다. 같은 그림인데 바이트가 매번
    #    달라진다(실측: 5,308,933 → 5,306,751 → 또 다름).
    #    대사 영상의 지문에는 **그 그림의 바이트**가 들어간다. 그래서 그림을
    #    한 장도 다시 안 그린 실행에서도 **대사 영상 14개가 전부 "안 맞는
    #    것"이 되어 다시 사졌다 — 한 번에 6,821원.**
    #    게다가 같은 자리를 겹겹이 뭉개서 그림도 조금씩 상한다.
    #    → 지운 그림 옆에 표시(.wiped)를 남기고, 있으면 건너뛴다.
    #      새로 그리면 short90.stills 가 그 표시를 지운다(다시 지우게 된다).
    n = keep = 0
    for f in sorted(d.glob("c*.png")):
        mark = f.with_suffix(".wiped")
        if mark.exists():
            keep += 1
            continue
        if wipe(f) is not None:
            ...
            n += 1
    if keep:
        print(f"■ 이미 지운 그림 {keep}장은 **손대지 않았습니다** "
              f"(손대면 대사 영상을 다시 사게 됩니다)")
    if n:
