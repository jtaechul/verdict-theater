...
# ⚠️ 2026-08-17 — GitHub 무료 실행시간(월 2,000분)이 바닥나 버튼 전부가 멈췄다.
#    4K 시트 11장 전수 검사는 이 파일에서 가장 오래 걸리는 부분인데, 그림도
#    그림 규칙도 안 바뀐 커밋에서는 결과가 같을 수밖에 없다. 그런 커밋에서만
#    자체 점검이 --light 로 여기를 건너뛴다(어느 파일이 바뀌면 전부 재야 하는지는
#    selfcheck.yml 의 IMG_PAT 목록이 정한다). 사람이 직접 돌릴 때는 늘 전부 잰다.
sheets_dir = ROOT / "assets" / "sheets"
if "--light" in sys.argv:
    print("   (건너뜀 — 그림·그림 규칙이 안 바뀐 커밋. 바뀌는 커밋에서는 전부 잰다)")
elif SG is None:
