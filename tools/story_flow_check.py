...
def thin_script():
    """2026-09-04 에 AI 가 실제로 지어 온 **얇은 대본** 그대로.

    손님: "영상이 너무 짧은데? 두편이야? … 너무 빠르게 본론으로 들어가
    버리니까 내용이 이해가 안 돼." 그때 나온 것이 이것이다.
    2편 15컷 389자 · 말줄임표 15개 · 남편과 내연녀가 얼굴 없이 목소리부터.
    """
    ...
    cuts = [
        c(1, "나레이션", "아내의 의심은 남편 차에 녹음기를 숨기며 시작되었습니다.",
          ["아내"]),
        c(2, "아내", "이거... 진짜야...?"),
        c(3, "나레이션", "녹음기 속 남편과 낯선 여자의 낯뜨거운 대화. "
                        "아내는 세상이 무너졌습니다.", ["아내"]),
        c(4, "남편", "아니... 그게 아니고... 오해야..."),
        c(5, "아내", "오해...? 내 귀로 다 들었는데..."),
        c(6, "나레이션", "분노한 아내는 상간녀를 상대로 삼천만 원의 소송을 "
                        "제기합니다.", ["아내"]),
        c(7, "내연녀", "그거... 불법 녹음인 거 아시죠...?"),
        c(8, "나레이션", "오히려 당당한 상간녀의 태도. 실제로 있었던 사건입니다.",
          ["내연녀"]),
        c(9, "나레이션", "상간녀가 아내를 상대로 삼천만 원을 물어내라며 "
                        "맞소송을 걸었습니다.", ["내연녀"]),
        c(10, "내연녀", "허락 없이 남의 대화 훔쳐듣는 거... 범죄거든요..."),
        c(11, "아내", "남의 가정 박살 내놓고... 뭐...?"),
        c(12, "나레이션", "상간녀는 아내의 행동도 사생활 침해라며 억지를 "
                         "부렸습니다.", ["내연녀"]),
        c(13, "내연녀", "경찰 부르기 전에... 당장 나가세요..."),
        c(14, "나레이션", "법원은 상간녀에게 위자료 삼천만 원을 아내에게 "
                         "지급하라고 판결합니다.", ["아내"]),
        c(15, "나레이션", "다만 아내의 불법 녹음 책임도 물어 오십만 원을 "
                         "깎아주었습니다.", ["아내"]),
    ]
    return {
        "title": "차량 녹음기 불법행위 반소 사건",
        "series_label": "녹음기와 맞소송",
        "hook": "남편 차에 녹음기를 숨겼다", "people": {}, "cuts": cuts,
        "parts": [
            {"no": 1, "cuts": [1, 8], "card": ["남편 차에서", "소리가 났다"],
             "yt_title": "남편 차에 숨긴 녹음기에서 들린 충격적 대화, 아내의 반격"},
            {"no": 2, "cuts": [9, 15], "card": ["증거를 들이대자", "돌아온 맞소송"],
             "yt_title": "불법 녹음이라며 3천만 원 맞소송 건 상간녀, 법원의 기막힌 판결"},
        ],
    }
...
def fake_s91():
    ...
    for part in range(3):
        ...
        add("나레이션", f"{part + 1}막이 열리던 그날의 이야기를 처음부터 차근차근 "
            f"짚어 가며 지금부터 전해 드리겠습니다", ["아내"])
        # ⚠️ 2026-09-17 — 여기서 **남편** 을 불러 주어야 한다. 1편 앞머리에서는
        #    나레이션이 이름을 불러 준 사람만 말할 수 있다(위 ④-2-3). 이 줄이
        #    '아내' 만 부르고 있어서, 컷4의 남편 대사가 그대로 걸렸다.
        add("나레이션", f"그 무렵 아내가 남편에 대해 무엇을 알게 되었는지부터 "
            f"차분히 짚어 보아야 합니다 {part}", ["아내", "남편"])
        add("아내", f"차에서 나온 그 목소리를 나는 {part}번이나 되돌려 들었어요")
        add("남편", f"그건 {part}년도 더 된 일이고 지금은 아무 사이도 아니라니까")
        add("나레이션", f"그러나 남편이 한 말과는 달리 통신사에서 받아 온 통화 "
            f"기록에는 {part}년치가 고스란히 남아 있었습니다")
        add("아내", f"통화가 {part}백 번이 넘는데 아무 사이가 아니라는 말인가요")
        add("남편", f"그 사람 이름은 이 집에서 {part}번도 꺼내지 말라고 했잖아")
        add("나레이션", f"아내는 결국 그날 밤 {part}시가 넘은 늦은 시각에 변호사 "
            f"사무실을 직접 찾아가 상담을 받았습니다")
        add("아내", f"소장에 적을 금액은 {part}천만 원으로 하겠다고 말했습니다")

    # ⚠️ 2026-09-05 — 예전에는 이 줄이 "원고의 청구를 기각한다" 였다.
    #    나레이션 주어 못이 생기면서 **주어가 없다**고 하나 더 걸렸다.
    #    이 시험글이 심는 흠은 둘(법원 화자 · 짧은 제목)뿐이어야 한다.
    #    말을 고치는 것은 AI 가 할 일이라 여기서는 성한 문장으로 둔다.
    cuts[12] = cut(13, "법원",
                   "재판장은 원고의 청구를 기각한다고 밝혔습니다")   # ← 여기서 걸렸다
    cuts[12]["who"] = ["법원"]
    return {
...
def main():
    ...
    # ⭐ 2026-09-09 — 제목 지시를 실측으로 다시 썼다(후킹 세 가지 중 하나).
    #    문구가 "22자보다 짧으면 안 된다" → "22~40자" 로 바뀌었다.
    ck(f"제목 아래 선이 프롬프트에도 적혀 있다 ({ST.TITLE_MIN}자)",
       f"{ST.TITLE_MIN}자" in md)
    ...
    # ⭐ 2026-09-08 — 하한을 **초**로 본다. 글자만 보면 컷 수 변화를 못 잡는다.
    ck("짧게 써도 잡힌다 (초 하한이 있다)", ST.PART_SEC_MIN >= 44)
    ck("하한과 상한 사이가 너무 좁지 않다 (8초 이상)",
       ST.PART_SEC_MAX - ST.PART_SEC_MIN >= 8,
       f"{ST.PART_SEC_MIN}~{ST.PART_SEC_MAX}초 — 모델이 빠져나갈 자리가 없다")
    ...
    # ⚠️⚠️ 2026-09-05 — 여기서 **살아 있는 대본 파일(S91)** 을 읽고 있었다.
    #    손님이 그 대본을 다시 지으시자 규격을 다 지키게 되어, 시험이
    #    "문제를 잡는다" 를 못 보고 빨간불이 났다. 시험은 손님이 바꾸는
    #    데이터에 기대면 안 된다 — **2026-09-04 에 실제로 나왔던 그 모양**을
    #    여기 박아 둔다 (2편 · 175자 · 말줄임표 9개 · 7컷 · 얼굴 없이 말하기).
    s91 = thin_script()
    b91 = ST.check(s91, new=True)
    # ⭐ 2026-09-08 — 반려 문구가 "185자" 에서 "초를 버리고 있다" 로 바뀌었다.
    #    글자가 아니라 초로 재기 때문이다(잣대가 54% 틀려 있었다).
    for want in ("편이 2개다", "버리고 있다", "말줄임표", "컷이다"):
    ...
    # ⚠️ 2026-09-17 — '말하기 전 컷에 먼저' 는 나레이션=장소 규칙과 싸워
    #    지킬 수 없는 못이었다. 프롬프트도 같이 고쳤다 — 시키는 말과 재는 잣대가
    #    어긋나면 모델은 시킨 대로 쓰고 검사에서 반려돼 돈만 날아간다.
    ck("말하는 사람을 그 컷 화면에 세우라고 적었다",
       "말하는 사람은 **그 컷 화면에** 나와 있다" in md)
    # ⚠️ 2026-09-17 — 규칙이 정교해졌다. 사람을 아예 금지하는 것이 아니라
    #    **등장인물만** 허용한다(보통명사는 그대로 금지).
    ck("나레이션 컷 기본은 장소라고 적었다",
       "나레이션 컷은 기본이 **장소**다" in md)
    ck("사람을 세우려면 등장인물 이름으로만 적으라고 했다",
       "**등장인물 이름**으로만 적고" in md)
    ck("보통명사 사람은 절대 금지라고 적었다",
       "보통명사(man·woman·lawyer…)는 절대 금지" in md)
    ...
    ck("S91 에서 잡는다 — 짧은 것은 증상이라고 알려 준다",
       any("껑충 뛰고 있다는 뜻" in x for x in b91h), str(b91h[:2]))

    # ⭐⭐⭐ 2026-09-17 손님: "이 사건으로 쇼츠 만들기 눌렀는데 왜 아무것도
    #    생성 안하냐." S93 은 **등장인물 전원**이 이 못에 걸려 반려됐고 692원이
    #    날아갔다. 까닭은 대본이 나빠서가 아니라 **못 두 개가 싸우고 있어서**다.
    #      · 2026-09-10 : 나레이션 컷에는 사람을 안 세운다 (나레이션 = 장소)
    #      · 그 전까지   : 새 인물은 '말하기 전 컷' 에 먼저 나와야 한다
    #    사람이 나올 수 있는 자리가 대사 컷뿐이 되었으니, 새 인물의 첫 등장은
    #    언제나 '말하는 그 컷' 이다 — 지킬 수 없는 못이 된 것이다.
    #    (S90·S91·S92 는 09-10 **이전** 대본이라 나레이션 컷에 얼굴을 얹어
    #     통과했다. 규칙이 바뀐 뒤 처음 지은 S93 이 그대로 걸렸다.)
    #    → 재는 것을 뜻에 맞췄다: **말하는 그 컷에 얼굴이 있으면 된다.**
    #    ⚠️ 살아 있는 데이터(S93 파일)를 읽지 않는다 — 손님이 다시 지으시면
    #       시험이 재려는 것을 못 보게 된다. 그 모양을 여기 박아 둔다.
    def spk(n, who, face=None):
        return {"n": n, "who": list(face if face is not None
                                    else ([] if who == "나레이션" else [who])),
                "turns": [[who, f"{who} 가 그날 있었던 일을 {n}번째로 "
                                f"또렷하게 짚어 말했습니다"]],
                "say": ["담담하게 낮은 목소리로"],
                "scene": ("a lamp lights an empty chair in a dim room"
                          if who == "나레이션" else "a woman sits in a dim room")}
    base = {"title": "유류분 사건", "series_label": "유류분 사건",
            "hook": "두집살림", "people": {},
            "parts": [{"no": 1, "cuts": [1, 3], "card": ["가", "나"],
                       "yt_title": "x" * 30}]}
    # ① 말하는 컷에 얼굴이 있으면 통과해야 한다 (S93 이 걸리던 모양)
    ok93 = dict(base, cuts=[spk(1, "나레이션"), spk(2, "아내"),
                            spk(3, "나레이션")])
    ck("말하는 그 컷에 얼굴이 있으면 통과한다 (S93 이 여기서 막혔다)",
       not [b for b in ST.check(ok93, new=True) if "화면에 없는 채" in b],
       str(ST.check(ok93, new=True)))
    ck("나레이션 컷에 얼굴을 얹으라고 시키지 않는다 (나레이션 = 장소)",
       ok93["cuts"][0]["who"] == [] and ok93["cuts"][2]["who"] == [])
    # ② 진짜 잘못 — 얼굴이 아예 없는 채 목소리만 나오는 것은 여전히 잡는다
    no_face = dict(base, cuts=[spk(1, "나레이션"), spk(2, "아내", face=[]),
                               spk(3, "나레이션")])
    ck("얼굴 없이 목소리만 나오면 잡는다",
       any("화면에 없는 채" in x for x in ST.check(no_face, new=True)))
    fixlog = ST.autofix(no_face)
    ck("그것을 0원으로 고쳐 준다 (AI 를 다시 안 부른다)",
       not any("화면에 없는 채" in x for x in ST.check(no_face, new=True)),
       str(fixlog))
    ck("고치는 자리는 **말하는 그 컷**이다 (나레이션 컷을 안 건드린다)",
       no_face["cuts"][1]["who"] == ["아내"]
       and no_face["cuts"][0]["who"] == [], str(no_face["cuts"][:2]))

    # ⭐⭐⭐ 2026-09-17 손님: "초반이 대뜸 뛰는 대사가 나오는 일이 없도록
    #    핵심 규칙에 반영하고 무조건 반영시켜."
    #    옛 S93 은 1편에서 세 번 뛰었고 **세 번 다 같은 모양**이었다 —
    #    나레이션이 아직 이름도 안 불러 준 사람이 갑자기 말을 한다.
    #    ⚠️ 시험 거리를 살아 있는 데이터에서 읽지 않는다. 그 모양을 박아 둔다.
    jump = dict(base, cuts=[spk(1, "나레이션"), spk(2, "아내"), spk(3, "나레이션")])
    jump["cuts"][0]["turns"][0][1] = ("그 집에 무슨 일이 있었는지부터 "
                                      "차근차근 전해 드리겠습니다")     # '아내' 없음
    ck("1편에서 나레이션이 안 불러 준 사람이 말하면 잡는다",
       any("나레이션에 한 번도 안 나온 채" in x
           for x in ST.check(jump, new=True)), str(ST.check(jump, new=True)))
    # 나레이션이 이름을 불러 주면 통과해야 한다 (고치는 값이 싸다 — 한 줄이면 된다)
    okj = json.loads(json.dumps(jump))
    okj["cuts"][0]["turns"][0][1] = ("그 무렵 아내가 무엇을 겪고 있었는지부터 "
                                     "차근차근 전해 드리겠습니다")
    ck("나레이션이 이름을 불러 주면 통과한다 (한 줄만 고치면 된다)",
       not [x for x in ST.check(okj, new=True) if "나레이션에 한 번도" in x],
       str(ST.check(okj, new=True)))
    # ⚠️ 2편부터는 안 건다 — 이미 다 세워진 사람들이다. 거기까지 걸면 멀쩡한
    #    대본이 걸린다(못은 좁게 박는다는 것을 여기서 붙들어 둔다).
    late = dict(base, cuts=[spk(1, "나레이션"), spk(2, "나레이션"), spk(3, "아내")],
                parts=[{"no": 1, "cuts": [1, 2], "card": ["가", "나"],
                        "yt_title": "x" * 30},
                       {"no": 2, "cuts": [3, 3], "card": ["다", "라"],
                        "yt_title": "y" * 30}])
    late["cuts"][0]["turns"][0][1] = ("그 집에 무슨 일이 있었는지부터 "
                                      "차근차근 전해 드리겠습니다")
    late["cuts"][1]["turns"][0][1] = ("그날 밤의 일을 순서대로 짚어 "
                                      "말씀드리겠습니다")
    ck("2편에서는 안 건다 (이미 세워진 사람들이다)",
       not [x for x in ST.check(late, new=True) if "나레이션에 한 번도" in x],
       str([x for x in ST.check(late, new=True) if "나레이션에 한 번도" in x]))
    # 프롬프트에도 같은 말이 적혀 있어야 한다 — 시키는 말과 재는 잣대가
    # 어긋나면 모델은 시킨 대로 쓰고 검사에서 반려돼 돈만 날아간다
    ck("1편 앞머리 규칙을 프롬프트에도 적었다",
       "나레이션이 먼저 이름을 불러 준 사람만 말할 수 있다" in md)
    ck("에둘러 부르면 안 된다고 적었다", "에둘러 부르면 불러 준 것이 아니다" in md)
    ck("대사는 같은 자리의 상대에게 하라고 적었다",
       "그 자리에 같이 있는 상대에게 한다" in md)
    ...
    doc = fake_s91()
    # ⭐⭐ 2026-09-08 — 이 붙박이 대본이 **지금 규격 안에 있는지** 먼저 본다.
    #    길이 잣대를 고쳤더니 이 대본이 조용히 '너무 짧다'가 되어, 아래 시험이
    #    재려던 것(손보기·되받아 고치기)과 무관한 이유로 빨간불이 났다.
    #    붙박이 자료가 규격에서 밀려나면 여기서 먼저 알려 준다.
    for _p in doc["parts"]:
        _a, _b = _p["cuts"]
        _m = [c for c in doc["cuts"] if _a <= c["n"] <= _b]
        _s = ST.part_sec(_m)
        ck(f"시험용 대본 {_p['no']}편이 규격 길이 안이다 ({_s:.0f}초)",
           ST.PART_SEC_MIN <= _s <= ST.PART_SEC_MAX,
           f"{ST.PART_SEC_MIN}~{ST.PART_SEC_MAX}초여야 한다 — "
           f"붙박이 대본을 규격에 맞게 고쳐라")
    was = ST.check(doc)
