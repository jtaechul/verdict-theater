...
def main():
    ...
    # ⚠️ 2026-09-14 — 예전에는 "세 편 모두 예약 공개" 라는 **글자 그대로**를
    #    찾았다. 편 수를 세어서 적도록 고치자(partWord) 이 검사가 멀쩡한
    #    화면을 잡았다. 편 수는 partcount_check 가 보므로, 여기서는
    #    **"모두 예약 공개" 라는 뜻**만 본다.
    ck("올리기 안내에 '모두 예약 공개' 라고 적혀 있다",
       "모두 예약 공개</b>로 올라갑니다" in js)
    ck("확인 창에도 적혀 있다",
       "모두 예약 공개입니다 (지금 공개되는 편은 없습니다)" in js)
    ck("편 수는 글자로 박지 않고 세어서 적는다", "partWord(" in js)
    ...
    # ⚠️⚠️ 2026-09-06 — 이 검사가 **한 글자 차이로 새 나갔다.** 안내 카드는
    #    "첫 편은 지금 올리고" 였는데 확인 팝업은 "첫 편은 지금 공개되고" 였다.
    #    글자를 딱 하나만 못 박아 두니 팝업이 옛말 그대로 남아, 손님이 [세 편
    #    예약 공개로 올리기] 를 누르자 "첫 편은 지금 공개되고" 가 떴다.
    #    → 화면에 뜨는 글 **전부**에서 '지금 공개/지금 올리' 를 찾는다.
    stale = [w for w in ("첫 편은 지금 올리고", "첫 편은 지금 공개",
                         "지금 공개되고", "지금 올리고") if w in js]
    ck("화면 어디에도 '첫 편은 지금 공개' 라는 옛 안내가 없다", not stale,
       " · ".join(stale) + " — 실제로는 모든 편이 예약이다")
