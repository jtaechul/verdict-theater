...
def main():
    ...
    print("\n⑧ 시험이 초를 베껴 적어 두지 않는가")
    # ⚠️⚠️ 2026-09-02 — 말 빠르기를 1.08 → 1.20 으로 올리자 시험이 깨졌다.
    #    끝 프레임을 **3.8초로 박아** 두었는데 컷이 3.73초로 줄어, 그 자리에
    #    프레임이 없었다. 이 저장소에 이미 적힌 교훈이다 —
    #    **셈을 시험에 베껴 적지 않는다. 진짜 값을 재서 쓴다.**
    t2 = (ROOT / "tools" / "short90_dryrun.py").read_text(encoding="utf-8")
    ck("끝 프레임 자리를 진짜 길이에서 잰다",
       "dur_of(mp4)" in t2 and '(1, 3.8)' not in t2,
       "초를 박아 두면 빠르기를 바꿀 때마다 시험이 깨진다")
