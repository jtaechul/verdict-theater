...
#
# ⭐ 2026-09-17 손님 승인: 3,000 → 8,000원.
#    '전체 영상'(그림 먼저 → 모든 컷 영상)을 넣으면서 올렸다. 손님이 고른 것은
#    **편 하나씩** 만드는 방식이고, 한 편이 약 5,900~6,400원이다(S93 실측).
#    3,000원으로 두면 편 하나가 반쪽에서 멈춘다 — 막는 것이 아니라 고장이 된다.
#    ⚠️ 8,000원은 **한 편이 겨우 들어가는 크기**다. 잘못 눌러도 손해가 한 편에서
#       끊긴다. 네 편을 한 번에 사려면(약 24,000원) 여기를 또 올려야 하고,
#       그때는 손님께 다시 여쭙는다.
RUN_KRW = float(os.environ.get("VT_RUN_KRW", "8000"))
# ⭐ 2026-09-10 손님 승인: 25,000 → 30,000원.
#    S92 를 만들다 **33원이 모자라** 멈췄다 (24,967원 쓴 상태에서 132원짜리
#    그림 한 장이 필요했다). 이미 만들어 둔 그림 37장·목소리 38줄이 그
#    한 장 때문에 통째로 묶여 있었다.
#    처음엔 30,000 으로 올렸는데 **그래도 1,217원이 모자랐다** —
#    남은 일이 그림 1장(132원) + 대사 영상 8컷(6,118원) = 6,250원이었다.
#    24,967 + 6,250 = 31,217원. 그래서 32,000원.
#    ⚠️ 한도는 **손님이 말씀하실 때만** 올린다. 내가 임의로 올리지 않는다 —
#       한도는 "실수해도 크게 안 새는 선" 이라 소리 없이 올리면 뜻이 없어진다.
#    ⚠️ 그리고 "실제 드는 값보다 **조금만** 위" 에 둔다. 넉넉히 잡으면
#       막는 시늉만 하는 것이다 (이 파일 위쪽 규칙).
#    2026-09-11 손님 승인: 32,000 → 39,000원.
#    대사를 20자로 줄여 4초 영상으로 바꾸면서, 어제 만든 8컷(5,645원)은
#    대사가 바뀌어 다시 사야 한다. 대신 대사 컷 **14개 전부**가 영상이 되고
#    (예전엔 8개) 한 컷 값은 706~941원 → 470원으로 내려간다.
#        이번 달 쓴 돈 31,231 + 새로 필요한 값 6,816 = 38,047원 → 39,000원
#    ⚠️ 38,000 으로 하려다 **47원이 모자라** 또 멈출 뻔했다. 9월 10일에도
#       33원이 모자라 멈춘 적이 있다 — 한도는 **셈해서** 정한다.
#
#    2026-09-14 손님 승인("진짜 마지막이야. 한도 올려."): 39,000 → 42,000원.
#    S92 의 대사 컷 넷(컷2·5·15·34)만 다시 산다. 나머지 열 개는 눈으로 맞춰
#    보고 살렸다(0원). 그림 38장은 이미 다 있고 대본과 맞는다(0원).
#        이번 달 쓴 돈 38,825 + 이번에 나갈 값 1,882 = 40,707원
#        + 안전 필터에 걸려 다시 만들 여유 470 × 2 = 940원
#        → 41,647원 이상이어야 한다 → 42,000원
#    ⚠️⚠️ 41,000 으로 적으려다 또 모자랄 뻔했다(여유 293원 — 재시도 한 번도
#       못 덮는다). **세 번째**다. 한도를 말하기 전에 반드시 셈부터 한다.
#    ⚠️ 한도는 "이만큼 써도 된다" 가 아니라 **"여기서 멈춘다"** 는 선이다.
#
#    2026-09-14 손님 승인("일단 한도 잠깐 풀어봐"): 42,000 → 47,000원.
#    **잠깐** 푸는 것이다 — S92 의 못 산 대사 컷 여덟 개를 채우기 위한 것.
#    다 채우면 다시 내려도 된다.
#        이번 달 쓴 돈 41,881 + 이번에 나갈 값 3,763 = 45,644원
#        + 안전 필터에 걸려 다시 만들 여유 470 × 2 = 940원
#        → 46,584원 이상이어야 한다 → 47,000원 (여유 416원)
#    ⚠️ 앞선 6개(2·5·6·8·13·18)는 **그대로 쓴다**(0원). 9월 13일에 살리는
#       장치를 조립 쪽에만 넣어 두었던 것을 사는 쪽에도 넣어 고친 결과다.
#    2026-09-17 손님 승인("이제 한도 다 올려놔"): 47,000 → 76,000원.
#    **전체 영상**(그림 먼저 → 모든 컷 영상)을 넣었고, S93 을 그 방식으로
#    끝까지 만들려면 다음이 필요하다. 셈은 tools/plan_cost.py 와 같은 자리다.
#        그림 16장(서로 다른 지문)            2,117원
#        목소리 1,146자                         842원
#        전체 영상 4편 (37컷 · 206초)        24,226원
#        안전 필터에 걸려 다시 만들 여유       1,882원 (8초짜리 × 2)
#        ─────────────────────────────────────────────
#        S93 완성에 필요                     29,067원
#        이번 달 이미 쓴 돈                  46,333원
#        합계                                75,400원  → 76,000원 (여유 600원)
#    ⚠️ 넉넉히 잡지 않았다. 이 파일 위쪽 규칙 그대로 **실제 드는 값보다 조금만**
#       위다. 넉넉히 잡으면 막는 시늉만 하는 것이다.
#    ⚠️ 셈하고 적는다 — 9월 10일(33원)·11일(47원)·14일(293원) 세 번 모자랐다.
#    ⚠️ 한 번 실행 뚜껑(RUN_KRW)은 그대로 둔다. 한 달 한도를 올려도 **한 번에**
#       나가는 돈은 한 편에서 끊겨야 한다 — 잘못 눌렀을 때 손해를 막는 선이다.
MONTH_KRW = float(os.environ.get("VT_MONTH_KRW", "76000"))
...
# ── 목소리 값 (100만 자당 달러) ────────────────────────────
#
#   ⚠️ 2026-08-22 — 여기도 **통째로 비어 있었다.** 그림값이 새어 나가던 것과
#      똑같은 자리다. 한국어 목소리를 무료 등급(하루 10번)에서 결제 계정으로
#      옮긴 순간부터 쓴 만큼 값이 나가는데, 장부에 한 줄도 안 남고 있었다.
#      무료일 때는 드러나지 않다가, 결제를 걸면 그때부터 새어 나간다.
#
#   ⚠️ 모르는 목소리는 **가장 비싼 값으로 친다.** 적게 잡으면 한도가 안 걸려
#      막는 시늉만 하게 된다. 실제 청구서를 한 번 보고 실측으로 고치면 된다.
# ⚠️⚠️ 2026-08-22 — 운영자의 실제 청구서: **28일에 38,200원.**
#    나는 "16화 전체 210원" 이라고 말했다. 아래 표가 틀렸기 때문이다.
#    글자 수로 값을 매겼는데 제미나이 목소리는 **토큰**으로 센다.
#    소리 1초가 토큰 몇 개인지 나는 모른다 — 그래서 통째로 빗나갔다.
#
#    → 이제 이 표는 **마지막 수단**이다. 구글이 알려 주는 실제 토큰 수
#      (usageMetadata)가 있으면 그것으로 매긴다 (tts.bill_flush).
#    → 그리고 **아주 비싸게** 잡는다. 적게 잡으면 한도가 안 걸려 막는
#      시늉만 하게 되고, 그 결과가 이번 일이다.
VOICE_USD_1M = [
    # (이름 조각,   100만 자당 달러)
    # ⚠️ 실측이 아니다. 실제로는 훨씬 비쌌다 → 열 배로 올려 잡는다.
    ("gemini",    300.00),   # 제미나이 목소리 — 모르면 비싸게
    ("chirp3",     30.00),
    ("chirp",      30.00),
    ("neural2",    16.00),
    ("wavenet",    16.00),
...
VOICE_USD_1M_UNKNOWN = 500.00   # 모르는 것은 가장 비싸게


def voice_krw(model, chars):
    """목소리 값(원). 글자 수로 센다."""
    m = str(model or "").lower()
    usd = next((u for k, u in VOICE_USD_1M if k in m), VOICE_USD_1M_UNKNOWN)
