...
# ⚠️ 2026-08-09 **낮추기로 고치는 길은 접었다.** 손님이 다섯 단계를 다 들어보시고:
#      "이대로는 안될 거 같아. 그냥 여자 목소리를 변형한 것처럼 이상하게 들려."
#    맞는 판단이다. 여자 음역 목소리를 남자 음역까지 끌어내리려면 너무 많이 내려야 하고,
#    그만큼 내리면 '낮은 여자 목소리' 가 될 뿐 '아저씨' 가 되지는 않는다.
#    → 목소리 이름 자체를 남자 목소리로 바꿨다(tts.py 의 VOICE_NAME 참고).
#    기능은 남겨 둔다 — 반음 한둘을 다듬는 데는 여전히 쓸모가 있고 값이 0원이다.
VOICE_PITCH = {
    "v_M50A": float(os.environ.get("VT_PITCH_M50A", "0")),
}
