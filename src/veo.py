...
# ⚠️⚠️⚠️ 2026-09-09 — **여기서 같은 실수를 두 번 했다. 이제 추측하지 않는다.**
#
#   2026-09-05: 1080p 로 4초를 사려다 거절당했다.
#       HTTP 400 "1080p is not supported for a duration of 4 seconds."
#     그때 "그러면 6초부터는 되겠지" 라고 **짐작해서** RES_MIN_SEC_1080=6 을
#     박았다. 확인은 안 했다 — 실제로 사는 것은 늘 4초라 그 길로 안 갔다.
#   2026-09-09: 대사 컷(6초)을 사려다 똑같이 거절당했다.
#       HTTP 400 "1080p is not supported for a duration of 6 seconds."
#     짐작이 틀렸던 것이고, 세 달 동안 아무도 몰랐다.
#
#   → 이제 **직접 사 보고 통과한 조합만** 쓴다. 아래 표에 없는 길이는
#     묻지도 따지지도 않고 720p 로 간다. 값은 초당으로 매겨지므로 화질을
#     내려도 값은 같다 — 잃는 것은 선명함뿐이고, 얻는 것은 "안 죽는다" 이다.
#
#   ⚠️ 여기에 길이를 새로 적으려면 **그 길이로 실제로 사서 성공한 뒤에** 적어라.
#      구글 문서나 짐작으로 적지 마라. 두 번 그렇게 해서 두 번 죽었다.
RES_OK = {
    # 화질: 실제로 사서 통과한 길이(초)들
    "720p": (4,),            # 2026-09-05 편 첫 장면 3개 성공
    "1080p": (),             # 아직 하나도 없다 (4초·6초 둘 다 거절당했다)
}


def res_for(sec):
    """그 길이로 **실제로 통과해 본** 화질. 모르면 720p 로 내린다."""
    if RESOLUTION != "720p" and int(float(sec)) in RES_OK.get(RESOLUTION, ()):
        return RESOLUTION
    return "720p"
