...
# ⭐⭐⭐ 2026-09-08 — **잣대가 54% 틀려 있었다. 편마다 13~16초를 버리고 있었다.**
#
#   손님(9/5): "영상이 너무 짧은데? 너무 빠르게 본론으로 들어가 버리니까
#              내용이 이해가 안 돼."
#   그때 나는 대본 탓이라고 보고 하한(185자)을 세웠다. 아니었다. **재는 자가
#   틀려 있었다.**
#
#   옛 잣대 `SEC_PER_CHAR = 0.248` 은 2026-09-01 값인데, 그날 말 빠르기
#   (short90.SPEED)가 1.08 이었다. 그 다섯 시간 뒤 1.20 으로 올렸는데 잣대는
#   안 고쳤다. 그래서 —
#       S91 1편 257자 → 옛 잣대는 63.7초라 하고, **실제는 41.3초**였다.
#   225자 상한이 실제로는 38초짜리 벽이었던 셈이다. 60초까지 22초가 남는데
#   검사가 그 앞에서 막고 있었다.
#
#   → 실측 27컷으로 다시 맞췄다 (최소제곱 · 최대 오차 1.6초):
#         컷 한 개 = 0.1218 x 글자 + 1.875초
#     세 편 모두 예측이 실제와 1.6초 안에 맞는다 (41.3/45.5/42.3 → 41.4/43.9/43.8).
#   ⚠️ 글자 수는 chars() 가 세는 방식(띄어쓰기·점 뺀 것)으로 잰 것이다.
#      다른 방식으로 세면 이 상수는 틀린다.
#
...
PART_CHARS_MIN = 200             # 마지막 울타리 (진짜 잣대는 PART_SEC_MIN 이다)
...
def autofix(doc):
    ...
    # ⑤ 말하는 사람은 **그 컷 화면에 세운다** (0원)
    #    ⭐⭐⭐ 2026-09-17 손님: "이 사건으로 쇼츠 만들기 눌렀는데 왜 아무것도
    #    생성 안하냐." 목소리만 있고 얼굴이 없으면 그림 모델이 아무 얼굴이나
    #    지어낸다 — ④ 와 같은 까닭이고, 대사 컷이므로 세우는 것이 맞다.
    #    ⚠️ 나레이션 컷은 건드리지 않는다 (나레이션 = 장소 · 2026-09-10).
    OK5 = set(who_ok(doc))
    for c in doc.get("cuts") or []:
        if is_narr_cut(c):
            continue
        who = list(c.get("who") or [])
        add = [w for w, _ in (c.get("turns") or [])
               if w != "나레이션" and w in OK5 and w not in who]
        for w in add:
            if w not in who:
                who.append(w)
                log.append(f"컷{c.get('n')}: 말하는 '{w}' 를 화면에 세웠다 "
                           f"(얼굴이 없으면 낯선 얼굴이 그려진다)")
        c["who"] = who
...
def check(doc, new=True):
    ...
    # ── ⭐ 말하는 사람은 **그 컷 화면에 나와 있어야** 한다 ──────────
    #    S91 은 남편이 컷4, 내연녀가 컷7 에서 얼굴 한 번 없이 목소리부터
    #    나왔다. 보는 사람은 누가 말하는지 모른 채 대사를 듣는다.
    #
    #    ⚠️⚠️⚠️ 2026-09-17 — 이 못이 **앞 컷에 먼저 나오라**고 요구하고 있었다.
    #    그런데 2026-09-10 에 '나레이션 = 장소(사람 없음)' 규칙이 들어왔다.
    #    그 뒤로 사람이 나올 수 있는 자리는 **대사 컷뿐**이고, 대사 컷의
    #    첫 등장은 곧 그 사람이 말하는 컷이다 — 즉 이 못은 그날부터
    #    **지킬 수 없는 못**이 됐다. S90(09-01)·S91(09-05)·S92(09-09) 는
    #    나레이션 컷에 얼굴을 얹어 통과했을 뿐이고, 규칙이 바뀐 뒤 처음 지은
    #    S93(09-17) 은 **등장인물 전원**이 걸려 692원을 태우고 반려됐다.
    #    (손님: "왜 아무것도 생성 안하냐")
    #    → 두 규칙이 싸우면 검사가 버그를 지키게 된다. 2026-09-10 에 그림
    #      쪽에서 똑같은 일을 한 번 겪었다(still_check ①-3 참고).
    #    → 재는 것을 **뜻**에 맞춘다: 말하는 그 컷에 얼굴이 있으면 보는
    #      사람은 누가 말하는지 안다. 얼굴이 아예 없는 것만 잡는다.
    #      (그것도 autofix ⑤ 가 0원으로 세워 주므로 여기까지 오면 진짜 잘못이다)
    if new:
        ...
        for c in cuts:
            ...
            for w, _ in (c.get("turns") or []):
                if w != "나레이션" and w in OK and w not in on and w not in told:
                    bad.append(f"컷{c.get('n')}: '{w}' 가 화면에 없는 채 "
                               f"말을 한다 — 말하는 사람을 그 컷 화면에 세워라")
                    told.add(w)                   # 한 번만 알린다

    # ── ⭐⭐⭐ 1편 앞머리는 **대뜸 뛰지 않는다** (2026-09-17 손님 지시) ──
    #    손님: "나레이션하고 대사가 초반에 너무 빠르게 진행되다 보니까 잘
    #           이해가 안 되거든 … 앞으로 다시는 이런 일이 없도록 핵심 규칙에
    #           반영하고 무조건 반영시켜."
    #
    #    옛 S93 은 1편에서 세 번 뛰었고, **세 번 다 같은 모양**이었다 —
    #    나레이션이 아직 이름도 안 불러 준 사람이 갑자기 말을 한다.
    #      · 컷3 아내 : 그때까지 나레이션에 '아내' 가 한 번도 안 나왔다
    #      · 컷5 딸   : '딸' 도 마찬가지
    #      · 컷6 내연녀: 나레이션은 '그 여자' 라고만 했다
    #    보는 사람은 누구 이야기인지 모르는 채 대사부터 듣게 된다.
    #
    #    → 1편에서는 **나레이션이 먼저 이름을 불러 준 사람만** 말할 수 있다.
    #    ⚠️ 1편에만 건다. 2편부터는 이미 다 세워진 사람들이다 — 거기까지 걸면
    #       멀쩡한 대본이 걸린다(못은 좁게 박는다).
    #    ⚠️ 고치는 값이 싸다 — 나레이션 한 줄에 이름을 넣으면 된다. 컷을 새로
    #       끼워 넣을 필요가 없으므로 되받아 고치기(repair)로 충분히 고쳐진다.
    if new and parts:
        first = parts[0]
