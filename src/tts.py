...
# ⭐⭐ 2026-08-31 — **한 편 안에서 목소리가 바뀌는 것을 막는 잠금.**
#    say() 는 제미나이가 막히면 조용히 옛 구글 목소리(ko-KR-…)로 물러선다.
#    16화 더빙에서는 그게 옳다 — 소리가 아예 없는 것보다 낫기 때문이다.
#    그런데 90초 한 편에서는 **정반대로 나쁘다.** 열한 번째 줄부터 목소리가
#    바뀌면 같은 사람이 중간에 딴 사람이 되고, 영상은 멀쩡히 나와서
#    눈으로는 안 보인다. 차라리 **거기서 멈추는 편이 낫다** — 만든 데까지는
...
# ⭐⭐⭐ 2026-09-04 손님: "대사 목소리 감정이 너무 격하게 표현되지 않도록
#    코드에도 반영해줘."
#    위 MOOD 는 **격한 판**이다 — 목소리를 높이고, 몰아붙이고, 서슬 퍼렇다.
#    기계 목소리에 그렇게 시키면 연기가 되는 게 아니라 소리만 커져서
#    싸구려 더빙처럼 들린다. 한국 드라마의 싸움은 내지르지 않는다.
#    → 같은 자리(물음표·느낌표·화·울음)를 **눌러 담은 말**로 다시 적는다.
#    ⚠️ 격한 판을 지우지 않는다. 결(STYLES)로 갈아 끼울 수 있어야
#       "다시 세게 해 줘" 한마디에 되돌릴 수 있다.
# ⚠️ 여기 글은 **바라는 것만** 적는다. "목소리를 높이지 마" 처럼 적으면
#    모델이 부정을 흘려듣고 오히려 높인다 (이 저장소의 오랜 규칙).
MOOD_CALM = [
    (r"미안|잘못했|제발|용서|어떻게 나한테|어떻게 나에게|눈물|울고|미치겠",
     "목이 메어 말끝이 조금 흔들리는, 낮고 조용한 목소리로"),
    (r"미쳤|제정신|당장|꺼져|닥쳐|뻔뻔|쓰레기|짐승|어디서|감히|똑바로",
     "화를 속으로 눌러 담아, 낮고 서늘하게"),
    (r"[!]",
     "한 마디씩 힘을 실어, 낮고 또렷하게"),
    (r"웃기|우스|그래서 뭐|어디 한번|잘났|대단하",
     "옅게 웃음기를 섞어 비꼬듯, 조용하고 차갑게"),
    (r"[?]",
     "믿기지 않는다는 듯 조용히 되묻듯, 끝을 살짝만 올려"),
]
MOOD_CALM_BASE = "감정을 속으로 눌러 담아, 담담하고 나직하게"
...
STYLES = {
    ...
    "dry": {
        "name": "담백하게 (쇼츠 템포)",
        "how": ("감정을 속으로 눌러 담아 낮고 담담하게, "
                "군더더기 없이 조금 빠른 호흡으로"),
        "rate": 1.12,
        "voice_f": "Kore", "voice_m": "Orus",
    },
    # ⭐ 2026-09-04 — 기본으로 삼는 결. 줄마다의 결은 살리되(MOOD_CALM),
    #    그 판 자체가 눌러 담은 말이라 어디서도 내지르지 않는다.
    #    빠르기는 1.0 그대로다 — 조립할 때 이미 1.2배로 당기므로(short90.SPEED)
    #    여기서 또 올리면 발음이 뭉개진다(2026-08-22 에 겪은 일).
    "calm": {
        "name": "차분하게 (기본 · 눌러 담은 톤)",
        "how": None,                    # 줄마다 MOOD_CALM 이 정한다
        "add": None,                    # 세기를 얹지 않는다
        "mood": None,                   # 아래에서 MOOD_CALM 을 꽂는다
        "rate": 1.0,
        "voice_f": "Kore", "voice_m": "Orus",
    },
    "deep": {
...
# 결마다 쓸 **줄별 판**을 꽂아 둔다 (안 적은 결은 격한 판을 쓴다 — 옛 동작 그대로)
STYLES["calm"]["mood"] = MOOD_CALM
STYLES["calm"]["mood_base"] = MOOD_CALM_BASE

# ⭐ 2026-08-21 — 운영자가 귀로 고른 결은 "더 격하게 (막장 톤)" 였다.
# ⭐⭐ 2026-09-04 손님: "대사 목소리 감정이 너무 격하게 표현되지 않도록."
#    → 기본을 "차분하게" 로 내린다. 되돌리려면 이 한 줄을 "fierce" 로
#      바꾸면 된다 (또는 그때그때 VOICE_STYLE=fierce).
STYLE_DEFAULT = "calm"
...
def mood(text):
    ...
    ⚠️ 판(표)은 **결이 정한다.** 기본 결(calm)은 눌러 담은 판을 쓰고,
       "격하게(fierce)" 로 바꾸면 옛 격한 판이 그대로 돌아온다.
    """
    st = style_of()
    table = st.get("mood") or MOOD
    base = st.get("mood_base") or MOOD_BASE
    t = str(text or "")
    for pat, how in table:
        if re.search(pat, t):
            return how
    return base
...
def route_note():
    ...
    # ⚠️⚠️ 2026-08-31 — 여기가 **재 보지도 않고 단정하고 있었다.**
    #    "무료 등급은 하루 10번뿐" 이라고 못 박았는데, 10번은 **무료 등급**의
    #    숫자다. 우리 열쇠가 무료인지 아닌지 여기서는 확인한 적이 없다.
    #    실제로 이 열쇠로 그림을 50장(7,531원) 만들었다 — 무료 등급은 그림을
    #    아예 못 만든다(429). 즉 **결제가 붙은 열쇠**이고, 하루 10번이 아니다.
    #    손님을 구글 클라우드 콘솔까지 보낼 뻔했다. 모르는 것은 모른다고 적는다.
...
def soft(text):
    ...
    ⭐ 2026-09-04 — "감정을 실어" 가 모델을 과하게 만들었다. 눌러 담으라고
       적는다 (손님: "감정이 너무 격하게 표현되지 않도록").
    """
    return (f'한국어 대사다. 감정을 속으로 눌러 담아 '
            f'또박또박 읽는다: "{bare(text)}"')
...
def say(text, voice=None, rate=1.0, pitch=0.0, out=None, style=None,
    ...
    try:
        ...
        if not gkey() or NO_FALLBACK:
            # ⚠️ NO_FALLBACK 이면 물러서지 않고 여기서 멈춘다 (위 설명 참조)
            raise
