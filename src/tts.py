...
VOICE_STYLE = {
    ...
    # ⭐ 2026-08-09 손님: "조금 더 묵직하고 아저씨 같고 걸걸한 목소리로."
    #    목소리 이름만 바꾸면 반쪽이다 — 지시문이 '서늘하고 느긋하게' 였는데
    #    그것은 걸걸한 아저씨와 정반대다. 몸집이 느껴지게 다시 썼다.
    "v_M50A":   ("쉰 살 맏아들. 목이 굵고 낮게 울리는 사람이다. 겉으로는 예의를 "
                 "갖추지만 속으로는 이미 결론을 내려놓았다. 배우가 연기하듯 "
                 "느긋하게, 말끝을 눌러 말한다.", 1.12),
...
VOICE_NAME = {
    ...
    #    '평탄한' 차남과 성격이 뚜렷이 갈린다.
    # ⭐⭐ 2026-08-09 오디션(30개를 실제로 재 봄)에서 **형제 둘 다 여자 음역**이었다.
    #     실측: Gacrux 172Hz · Schedar 184Hz — 어머니(212)·할머니(193) 쪽에 가깝다.
    #     주석에 적혀 있던 옛 값(186·129)은 **다른 모델로 잰 것**이라 크게 어긋나 있었다.
    #     손님이 "여자 목소리를 변형한 것처럼 들린다" 고 하신 것이 정확했다.
    #     이제 실제로 잰 표(data/voices.json)를 보고 고른다.
    "v_M50A":   "Algenib",      # Gravelly(걸걸한)  134.5Hz — 손님이 원하신 '걸걸한 아저씨'
    "v_M50B":   "Fenrir",       # 149.5Hz — 형과 15Hz 벌어져 섞여 들리지 않는다
...
def model_for(speaker):
    """그 배역의 목소리를 만드는 모델. **해설만 다른 모델을 쓴다.**"""
    return NARRATOR_MODEL if speaker == "narrator" else CHAR_MODEL_ORDER[0]
...
def effective_hz(speaker, table=None):
    ...
    ⚠️ **모델이 다르면 잰 값을 쓰면 안 된다.** 같은 이름이 모델에 따라 통째로
       달라진다 — 실측: Charon 이 2.5 에서 82Hz, 3.1 에서 157Hz 다.
       해설만 2.5 를 쓰는데 3.1 로 잰 값으로 판단하면 엉뚱한 결론이 난다.
       모델이 다르면 '모른다' 로 두고 경고만 한다 — 틀린 판정보다 낫다."""
    table = voice_table() if table is None else table
    row = table.get(VOICE_NAME.get(speaker, ""))
    if not row or not row.get("hz"):
        return None, 0.0
    if row.get("model") and row["model"] != model_for(speaker):
        return None, 0.0
    st = 0.0
