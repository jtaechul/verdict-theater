...
def recipe(speaker, model, text=""):
    ...
    way = "g4" if GROUP_ON else "s1"   # g4 = 말끝 올리기 허용
...
def synth_group(key, model, lines, speaker, out_mp3, rotate=False):
    ...
    prompt = ("\n".join(x for x in (
        ...
        # ⭐ 2026-08-07 손님 지적: "물음표는 말끝을 올리는 게 맞지 않아?" — 맞다.
        #    말끝을 올리는 것은 한국어에서 자연스럽다. 그걸 막으면 로봇이 된다.
        #    막아야 하는 것은 **줄 전체를 높은 목소리로 읽는 것**이다.
        #    실측: H05 는 말끝만이 아니라 줄 전체가 +8.5반음이었다 — 다른 사람이 됐다.
        #    그래서 '올리지 마라' 가 아니라 '말끝만 올리고 목소리 자체는 그대로' 라고 시킨다.
        "물음표로 끝나는 줄은 **말끝만** 자연스럽게 올려라."
        " 줄 전체를 높은 목소리로 읽지 마라 — 목소리 자체는 앞줄과 똑같아야 한다.",
