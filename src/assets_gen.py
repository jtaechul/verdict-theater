...
[크기]
아래 픽셀 값은 **넘으면 안 되는 상한**이다. 더 크게 그리지 않는다. 작게 그리는 것은 괜찮다.
얼굴을 크게 그린 여섯 사람: 머리 꼭대기부터 목 아래까지가 세로 **760픽셀을 넘지 않고**(그림 세로의 약 14%), 어깨는 그리지 않는다. 좌우 폭은 520픽셀을 넘지 않는다. 여섯 사람의 머리 크기가 서로 똑같다.
허리 위까지 그린 여섯 사람: 머리 꼭대기부터 허리까지가 세로 **760픽셀을 넘지 않고**(그림 세로의 약 14%), 두 어깨가 다 보인다. 허리 아래는 그리지 않으며, 잘린 단면을 표현하지 않고 자연스럽게 마무리한다. 좌우 폭은 560픽셀을 넘지 않는다. 여섯 사람의 머리 크기와 어깨 폭이 서로 똑같다.
...
똑바로 서 있는 사람은 머리 꼭대기부터 발바닥까지가 세로 **1400픽셀을 넘지 않는다**(그림 세로의 약 25%).
앉거나 주저앉은 사람은 몸 크기가 같으므로 자세 때문에 자연히 더 낮아진다. 억지로 확대하거나 축소하지 않는다.
한 사람의 좌우 폭은 640픽셀을 넘지 않는다.
