...
def process_sheet(sheet_path, code, kind=None, outdir=None, upscale=UPSCALE,
    ...
    # ⚠️ 2026-08-14 — 여기가 `if key:` 였다. 열쇠가 없으면 덩어리 방식을 아예
    #    건너뛰고 **격자 방식으로 갔다.** 선이 없는 새 시트에는 격자가 없으므로
    #    그건 반드시 어긋난다(한 칸에 사람 둘). 새 시트는 열쇠가 없어도 덩어리
    #    방식으로 간다 — 열쇠는 '어느 덩어리가 어느 표정인가' 를 눈으로 확인할
    #    때만 쓰고, 없으면 프롬프트에 적은 차례대로 짝짓는다(그 차례는
    #    tools/limit_test.py 가 프롬프트와 대조해 지킨다).
...
#
# ⭐⭐ 아래 프롬프트의 **픽셀 숫자는 읽히지 않는다** (2026-08-14 · 두 장을 재서 확인).
#        요구 세로 950 → 실제  992~1168   |  요구 세로 760 → 실제 1160~1264
#        요구  폭 620 → 실제  768~840    |  요구  폭 520 → 실제  752~760
#        요구  폭 720 → 실제      840    |  요구  폭 560 → 실제  880~888
#        하단 밴드 700 → 92px 침범        |  하단 밴드 900 → 1232px 침범
#     **작게 적었더니 오히려 더 크게 그렸다.** 모델은 늘 화면을 칸(3열×4줄)으로
#     나누고 칸의 72~92% 를 채운다. 숫자를 바꾸는 것으로는 크기가 안 바뀐다.
#
#     ⚠️ 그러니 **이 숫자들을 다시 만지지 마십시오.** 265원짜리 헛수고가 된다.
#        (2026-08-14 에 실제로 두 번 그렇게 썼다: 950→760 으로 줄여도 그대로였다)
#        지금 이 프롬프트로 나온 시트는 검사를 **통과했고 눈으로도 확인했다.**
#        크기를 바꾸고 싶으면 숫자가 아니라 **줄·열 수**를 바꿔야 한다.
#        숫자는 지운 것이 아니라 남겨 둔다 — 있어서 나쁠 것은 없고,
#        지웠다가 다른 것이 함께 무너지는 편이 더 위험하다.
...
사람과 사람 사이에는 300픽셀 이상 폭의 초록이 이어진다. 좌우 이웃끼리도, 위아래 무리끼리도 그렇다. 열두 사람은 서로 완전히 떨어져 있어 어디에서도 닿지 않는다. 두 사람이 한 곳이라도 닿으면 잘못 그린 것이다 — 특히 위 두 무리의 여섯 얼굴은 어깨가 없으므로 이웃과 닿을 것이 아예 없어야 한다.
어떤 사람도 이미지의 위·아래·왼쪽·오른쪽 끝에서 300픽셀 이상 안쪽에 있다. 머리 꼭대기 위에도 300픽셀 이상의 초록이 있다.

[크기]
아래 픽셀 값은 **지켜야 하는 크기**다. 더 크게도, 더 작게도 그리지 않는다.
얼굴을 크게 그린 여섯 사람: 머리 꼭대기부터 목 아래까지가 세로 **760픽셀**이다(그림 세로의 약 14%). 780픽셀보다 크면 안 되고 750픽셀보다 작아도 안 된다. 목 아래는 아무것도 그리지 않는다 — 어깨도, 정장도, 넥타이도, 셔츠 깃도 그리지 않는다. 여섯 얼굴은 목에서 끝나 초록 위에 뜬 머리이며, 위 두 무리에 옷이 한 조각이라도 보이면 잘못 그린 것이다. 좌우 폭은 520픽셀을 넘지 않는다(머리카락 폭이 곧 전체 폭이다). 여섯 사람의 머리 크기가 서로 똑같다.
