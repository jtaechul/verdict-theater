...
이미지 맨 아래쪽 700픽셀 높이만큼은 사람이 하나도 들어오지 않는, 초록만 있는 곳이다. 머리카락 한 올, 옷자락 하나도 그 아래로 내려오지 않는다.
사람과 사람 사이에는 200픽셀 이상 폭의 초록이 이어진다. 좌우 이웃끼리도, 위아래 무리끼리도 그렇다. 열두 사람은 서로 완전히 떨어져 있어 어디에서도 닿지 않는다.
어떤 사람도 이미지의 위·아래·왼쪽·오른쪽 끝에서 200픽셀 이상 안쪽에 있다. 머리 꼭대기 위에도 200픽셀 이상의 초록이 있다.

[크기]
얼굴을 크게 그린 여섯 사람: 머리 꼭대기부터 목 아래까지가 세로 약 950픽셀(그림 세로의 약 17%)이고, 어깨는 그리지 않는다. 좌우 폭은 620픽셀을 넘지 않는다. 여섯 사람의 머리 크기가 서로 똑같다.
허리 위까지 그린 여섯 사람: 머리 꼭대기부터 허리까지가 세로 약 950픽셀(그림 세로의 약 17%)이고, 두 어깨가 다 보인다. 허리 아래는 그리지 않으며, 잘린 단면을 표현하지 않고 자연스럽게 마무리한다. 좌우 폭은 720픽셀을 넘지 않는다. 여섯 사람의 머리 크기와 어깨 폭이 서로 똑같다.
...
이미지 맨 아래쪽 700픽셀 높이만큼은 사람이 하나도 들어오지 않는, 초록만 있는 곳이다. 아래 무리 두 사람의 발 아래로도 300픽셀 이상의 초록이 더 이어진다.
사람과 사람 사이에는 250픽셀 이상 폭의 초록이 이어진다. 좌우 이웃끼리도, 위아래 무리끼리도 그렇다. 다섯 사람은 서로 완전히 떨어져 있어 팔·다리·머리카락·옷자락 어디에서도 닿지 않는다.
어떤 사람도 이미지의 위·아래·왼쪽·오른쪽 끝에서 200픽셀 이상 안쪽에 있다. 머리 꼭대기 위에도, 발끝 아래에도 200픽셀 이상의 초록이 있다.

[크기]
다섯 사람은 모두 같은 몸 크기로 그린다. 머리 크기가 다섯 번 모두 똑같다.
똑바로 서 있는 사람은 머리 꼭대기부터 발바닥까지가 세로 약 1700픽셀(그림 세로의 약 31%)이다.
앉거나 주저앉은 사람은 몸 크기가 같으므로 자세 때문에 자연히 더 낮아진다. 억지로 확대하거나 축소하지 않는다.
한 사람의 좌우 폭은 800픽셀을 넘지 않는다.
