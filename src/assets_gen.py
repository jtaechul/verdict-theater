...
def char_sheet_prompt(code):
    return (
        ...
        + COLOUR_RULE_KO +
        # ⚠️⚠️ 2026-08-13 — 여기가 "3열 6행 격자" 한 줄뿐이었다. 그래서 모델이
        #    **위 4줄만 3칸으로 그리고 아래 2줄은 4칸으로** 그렸다(M70 실측).
        #    자르는 쪽은 3열 6행 균등으로 믿고 잘라서 아래쪽이 전부 어긋났다.
        #
        #    더 나쁜 것은 **격자선이 인물의 머리 위를 가로질러** 그어진 것이다.
        #    그 선을 지우면 머리 꼭대기가 같이 지워져 **목이 잘리고**,
        #    안 지우면 화면에 **사각 테두리**로 남는다. 둘 다 손님이 지적했다.
        #      "① 아직도 사각 테두리가 남아있어" "미친놈아 이건 목이 잘렸잖아"
        #
        #    ⭐ 자르는 코드를 아무리 똑똑하게 만들어도 이 시트는 못 살린다.
        #       **그리는 단계에서 막아야 한다.** 그래서 아래 세 줄을 못 박는다.
        #         · 모든 줄이 정확히 3칸 (줄마다 칸 수가 다르면 안 된다)
        #         · 칸마다 넉넉한 초록 여백 — 인물이 칸 선에 닿지 않는다
        #         · 선이 인물 위를 지나가지 않는다
        "- 3열 6행 = 18칸 격자. **모든 줄이 정확히 3칸**이다. "
        "줄마다 칸 수가 다르면 안 되고, 한 인물이 두 칸을 차지해도 안 된다\n"
        "- 칸의 크기가 18개 모두 똑같다. 가로줄·세로줄이 곧고 일정한 간격이다\n"
        "- **인물은 자기 칸 안에만 있고 칸 선에 닿지 않는다.** 인물 둘레(머리 위·"
        "발 아래·양옆)에 칸 크기의 8% 이상 되는 순수 초록 여백을 남긴다\n"
        "- **칸 선이 인물 위를 지나가면 안 된다.** 특히 머리 위로 선을 긋지 않는다\n"
