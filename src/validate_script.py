...
MAX_CUT_SEC = 7.0
MAX_TEXT_LEN = 45
# ⭐ 총 컷 수 (2026-08-11 손님 선택으로 상한을 115 → 120 으로 넓힘)
#
#    EP002 는 채점 99점으로 완성됐는데 컷이 118개라 이 한 줄에만 걸렸다.
#    그 오류 1건 때문에 stage 가 'scripting' 이 되고, 관리자 화면이 그걸
#    '덜 만들어졌다' 로 읽어 무한 실패 고리가 생겼다.
#    12분(720초)에 118컷이면 컷 하나가 평균 6.1초다. 길지도 짧지도 않은
#    정상 범위이므로, 멀쩡한 대본을 떨어뜨리던 상한 쪽을 넓힌다.
CUT_MIN, CUT_MAX = 100, 120
