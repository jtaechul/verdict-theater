...
# ⭐⭐⭐ 2026-09-06 손님: "조회수에 부정적인 영향을 미치는 것만 당장 없애고,
#    긍정적인 영향을 미칠 만한 해시태그를 넣도록 코드 수정해."
#
#    ■ 실측 (vidIQ · 한국 검색량 · 2026-09-06)
#        사연        666,590      막장드라마   66,734
#        불륜         61,342      남편        41,276
#        이혼사연     38,434      부부        38,272
#        사이다사연   36,400      반전사연    35,163
#      ⚠️ 그때까지 쓰던 것 가운데 **검색량이 안 잡히는 것**:
#         법률사연 · 쇼츠드라마 · 실화사연 · 외도 · shorts
#         → 뺐다. 태그가 많다고 좋은 게 아니라, 뜻 없는 태그는 묽게 만든다.
#      ⚠️ 사연극장·실화사건은 **남긴다** — 2026-09-04 에 손님이 넣으라고
#         하신 것이다. 다만 뒤로 민다.
#
# ⭐ 맨 앞에 두는 것 — 유튜브가 제목 위에 **클릭할 수 있게 띄우는 것은 앞의
#    세 개**뿐이다. 그래서 가장 큰 것을 맨 앞에 둔다.
LEAD_TAGS = ["사연"]                       # 666,590 — 압도적 1위 · 늘 맞는 말

# 채널이 늘 달고 가는 것 (주제 태그 뒤에 붙는다)
...
TOPIC = [
    ...
    # ⚠️ '외도' 는 검색량이 안 잡혀 뺐다. '이혼사연'(38,434)으로 바꾼다.
...
def make(doc, no):
    ...
    #    있는 거야?" — 없다. 유튜브는 2022년부터 **세로 9:16 + 짧은 길이**로
    #    쇼츠를 스스로 알아본다. 제목의 #shorts 는 아무 일도 하지 않으면서
    #    제목 100자 가운데 8자를 먹고, 쇼츠 화면에서 제목이 잘리는 자리를
    #    잡아먹는다. → **제목에서는 뺀다.** (설명 해시태그에는 그대로 둔다)
    #    ⚠️ 붙이던 줄은 지웠다. `if False:` 로 남겨 두면 언젠가 되살아난다.
    ...
    tags = [t for t in LEAD_TAGS if t not in tags] + tags
    for b in BASE_TAGS:
        if b not in tags:
            tags.append(b)
    tags = tags[:TAG_MAX]
...
def part_meta(doc, part, last=False):
    ...
    # ⚠️⚠️⚠️ 2026-09-09 — **뺐다가 되돌렸다. 이 채널의 숫자가 그렇게 말한다.**
    #
    #   2026-09-06 에 내가 "유튜브는 세로 9:16 으로 쇼츠를 스스로 알아보니
    #   제목의 #shorts 는 하는 일이 없다" 며 뺐다. 일반론으로는 맞는 말이다.
    #   그런데 **이 채널 12편의 실측은 정반대였다** (2026-09-09):
    #
    #       제목에 #shorts 있음  9편 —   353 ~ 3,576회
    #       제목에 #shorts 없음  3편 —    46 ~   220회
    #       → 두 무리가 **겹치는 구간이 하나도 없다** (없음 최고 220 < 있음 최저 353)
    #
    #   뺀 뒤 올린 세 편(S91)이 46·72·220회로 무너졌다. 그 전 아홉 편은
    #   전부 1,214회 이상이었다. 3편은 적은 표본이고 S91 에는 다른 사고도
    #   겹쳐 있었지만(지웠다 다시 올림 · 1편이 첫 두 시간을 옛 제목으로
