...
def looks_human(v):
    """주소 설명에 사람 낱말이 있는가 (첫 번째 그물)."""
    words = set(slug_of(v).split())
...
def search(query, per=15):
    ...
    req = urllib.request.Request(f"{API}?{q}", headers={"Authorization": k})
    try:
        with urllib.request.urlopen(req, timeout=20) as r:
            return json.loads(r.read().decode("utf-8")).get("videos") or []
    except Exception as e:                                   # noqa: BLE001
        print(f"  ⚠️ 스톡 검색을 못 했다 ({e}) — 그림으로 갑니다")
        return []


def best_file(v):
    """세로에 가장 잘 맞는 파일 하나. 너무 작은 것은 버린다."""
    got = [f for f in (v.get("video_files") or [])
...
def has_person(png):
    ...
    try:
        import bg_fetch as BF
        judge = getattr(BF, "ask_gemini_person", None)
        if callable(judge):
            return bool(judge(png))
    except Exception:                                        # noqa: BLE001
        pass
...
def fetch_one(c, out, dry=False):
    ...
    for v in cand[:4]:                       # 위에서부터 네 개까지만 본다
        ...
        if frame_of(out, png) and has_person(png):
            print(f"    · 사람이 보인다 — 버린다 ({slug_of(v)[:38]})")
            out.unlink(missing_ok=True)
            continue
        png.unlink(missing_ok=True)
        tmp.unlink(missing_ok=True)
        return True, slug_of(v)[:40]
    tmp.unlink(missing_ok=True)
    png.unlink(missing_ok=True)
...
def main():
    ...
    if not key():
        print(f"  {KEY_ENV} 가 없습니다 — 아무것도 안 합니다. "
