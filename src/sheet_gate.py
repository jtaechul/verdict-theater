...
# ⚠️ 2026-08-16 — 0.68 이 **높았다.** 합격감 시트(눈으로 확인)의 얼굴이 칸의
#    65.1%(896px)로 그려졌는데 하한 68%(935px)에 걸려 265원이 격리됐다.
#    실측 범위를 0.65~0.92 로 넓히고 하한은 그 아래 여유(0.60)로 내린다.
#    0.60 이라도 4K 시트에서 얼굴 826px — 컷아웃 화질은 넉넉하다.
FILL_LO = 0.60                     # 칸을 최소 이만큼은 채워야 한다 (실측 0.65~0.92)
...
KINDS = {
    ...
    #   열둘이 다 같은 크기로 그려진다. 하한은 실측(65~92% 채움)보다 여유 있게.
...
def check(path, kind="face", verbose=True):
    ...
    def spanning(mask, axis):
        ...
        # ⚠️⚠️ 2026-08-14 (2차) — '몸 픽셀이 폭의 90% 넘게 퍼진 얇은 줄' 로 쟀더니
        #    사람 다리 겹침(69%)은 피했지만…
        # ⚠️⚠️⚠️ 2026-08-16 (3차 · 265원 오판) — **폭 936px 상반신 셋이 나란히 서면
        #    한 줄의 몸 픽셀이 91.4% 가 되어** 멀쩡한 얼굴 시트가 "가로줄 1개" 로
        #    떨어졌다. '얼마나 퍼졌나' 는 결국 어림이었다.
        #    이제 **선의 정의 그대로** 잰다 — 격자선은 사람 사이의 **빈 배경 복도**
        #    까지 가로지른다. 사람은 제 칸 안에만 있으므로 복도에 몸 픽셀을 못
        #    남긴다. 복도(모든 인물 상자 밖 구간)에 절반 넘게 픽셀이 깔린 얇은
        #    구간만 '줄' 로 본다. 진짜 선(끝에서 끝)은 여전히 걸리고,
        #    넓은 사람들은 원리적으로 못 걸린다.
        W_ = mask.shape[axis]
        # 인물 상자들이 그 축에서 차지하는 구간의 여집합 = 빈 복도
        used = []
        for _a, bx0, by0, bx1, by1 in bs:
            lo_, hi_ = (bx0, bx1) if axis == 1 else (by0, by1)
            used.append((max(0, lo_ - 4), min(W_, hi_ + 4)))
        used.sort()
        corridors, cur = [], 0
        for lo_, hi_ in used:
            if lo_ > cur:
                corridors.append((cur, lo_))
            cur = max(cur, hi_)
        if cur < W_:
            corridors.append((cur, W_))
        cols = [c for lo_, hi_ in corridors for c in range(lo_, hi_) if hi_ - lo_ >= 10]
        if len(cols) < W_ * 0.05:
            # 복도가 없다 = 덩어리가 폭을 다 덮었다(붙음). 그건 G3·G7 소관이다.
            return 0, 0
        sel = (mask[:, cols] if axis == 1 else mask[cols, :].T)
        cover = sel.sum(axis=1)
        wide = np.where(cover > len(cols) * 0.5)[0]
        if len(wide) == 0:
            return 0, 0
