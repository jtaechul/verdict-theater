...
- **한 컷 대사는 24~28음절을 목표로 한다** (최소 20 · 최대 30).
  공백·쉼표는 소리가 안 나므로 **음절로 센다.**
  (`"여기가 어디라고 뻔뻔하게 와?"` = 12음절 · 2.2초 — **이건 너무 짧다**)
  한국어 드라마 대사는 초당 5~6음절이고 6초 중 5.5초를 말한다.
  20음절이면 3.6초, 28음절이면 5.1초다. **20음절짜리를 줄줄이 쓰지 말고
  24~28을 노린다** — 최소치에 붙여 쓰면 화면이 계속 빈 채로 나간다.
- **혼자 말하는 컷이 특히 짧아지기 쉽다.** 한 마디 던지고 끝내지 말고
  두 문장으로 잇는다.
  ✗ `"이대로 당할 줄 알아?"` (9음절 · 1.6초)
  ○ `"이대로 당할 줄 알아? 하나도 남김없이 다 찾아낼 거야."` (24음절 · 4.4초)
