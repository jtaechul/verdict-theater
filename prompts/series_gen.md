...
### ② 한 컷 대사는 **30~34음절**을 목표로 한다 (최소 16 · 최대 35)
...
**20음절짜리(3초)를 쓰면 6초 중 절반이 빈 채로 나간다.**
...
```
DIALOGUE: 아내 (furious): "병원까지 손댔어? 진짜 끝까지 가네. 사람이 어떻게 그래?" / 동거녀 (flat): "그이가 준 거야. 내가 뺏은 게 아니라."
caption: 사망 두 달 전, 병원 지분 12억이 동거녀 앞으로 넘어갔다
```

위 `DIALOGUE` 는 36음절 · 5.6초 — 6초를 꽉 채운다. 아래처럼 짧게 끝내면 화면이 빈다:

```
✗ DIALOGUE: 아내 (furious): "병원까지 손댔어?"        ← 9음절 · 1.4초뿐
```
...
- [ ] 한 컷 대사가 **30~34음절**쯤 되는가 (20음절짜리가 줄줄이 있지 않은가)
