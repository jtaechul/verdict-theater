...
DIALOGUE: 아내 (furious): "여기가 어디라고 뻔뻔하게 와?" / 동거녀 (calm): "마지막 가는 길인데 인사도 못 해요?"
```

위는 12 + 14 = **26음절**. 한쪽이 다 쓰지 말고 **반씩 나눈다.**

✗ 이렇게 나누어 놓으면 안 된다 — 한 컷에 한 사람씩만 말하고 다음 컷으로 넘김:
```
✗ 3컷 DIALOGUE: 아내 (furious): "여기가 어디라고 뻔뻔하게 와?"
✗ 4컷 DIALOGUE: 동거녀 (calm): "마지막 가는 길인데 인사도 못 해요?"
```

한 컷에 주고받는 횟수는 **최대 3번** (`"뭐?"` / `"들었잖아."` / `"야, 너!"`).

### ② 한 컷 대사는 **24~28음절**을 목표로 한다 (최소 20 · 최대 30)

공백·쉼표는 소리가 안 나므로 **음절로 센다.**
한국어 드라마 대사는 초당 5~6음절이고, 6초 클립에서 앞뒤 숨 쉴 틈을 빼면
약 5.5초를 말한다. 20음절이면 3.6초, 28음절이면 5.1초다.
**20음절짜리를 줄줄이 쓰지 말고 24~28을 노린다.**

`"여기가 어디라고 와?"` (9음절 · 1.6초) 처럼 한 마디 던지고 끝내면
6초 중 4초가 빈 채로 나간다.
...
- [ ] 한 컷 대사가 **24~28음절**쯤 되는가 (20음절짜리가 줄줄이 있지 않은가)
