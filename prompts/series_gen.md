...
| 컷 | 역할 | 무엇을 담나 |
|---|---|---|
| 1 | **터짐** | 3초에 멈춰도 무슨 상황인지 알게. **설명으로 시작 금지** |
| 2 | 맞섬 | 상대가 받아치고, **행동이 하나** 일어난다 (통장을 챙긴다·자리를 뜬다) |
| 3 | 선언 | **새 사실 하나가 터지고, 질문으로 끊는다** |

**컷1이 설명으로 시작하는 화는 실패다.** 넘기던 사람은 1초 안에 정한다.

---

# ⭐⭐ 한 화 = 한 사건 (2026-08-25 · 가장 중요)

> 운영자: "대본 읽어봤는데 스토리가 전혀 이해가 안돼. 씬 앞뒤로 개연성이 없어."
> 대조해 보니 **16화에 사건이 하나도 없었다.** 전부 말싸움이고, 16화 전부
> '선언(답)' 으로 끝나 다음 편을 볼 이유가 없었다.

세로 숏드라마(DramaBox 같은 것)의 회차 구조는 이렇다 —
**훅 → 충돌 → 폭로 → 버튼(질문으로 끊기).** 이 넷을 지킨다.

### ① 화마다 **새로 밝혀지는 것 하나**를 정한다 (`reveal`)

16화면 **서로 다른 사건 16개**다. 같은 사실을 두 화가 나눠 갖지 않는다.

```
2화 reveal: 법원이 남편의 이혼 청구를 기각했다
3화 reveal: 남편이 사망보험 아홉 건의 받을 사람을 그 여자로 바꾼다
7화 reveal: 남편의 통장에 남은 것은 빚 육억뿐이다
```

그 폭로가 **그 화 대사에 실제로 있는지** 검사기가 낱말로 확인한다(`must`).
그리고 **그보다 앞 화에서 미리 말하면 반려**한다 (김이 새기 때문).

### ② 마지막 대사를 **질문으로 끊는다** — 마지막 화만 예외

> *"cut on the question, not the answer"*

```
✗ 답으로 끝남 : "네가 훔쳐 간 돈 전부 다 찾아올 거야."   ← 다음 편을 볼 이유가 없다
○ 질문으로 끊음: "…무슨 종이요?"                        ← 다음 편을 봐야 한다
```

### ③ 사실은 **대사가** 지되, 설명이 아니라 **무기로** 넣는다

숏드라마는 설정 설명을 화면에 안 붙인다. 인물이 **상대를 찌르려고** 사실을 말한다.

```
✗ 설명체: "2013년에 법원이 당신의 이혼 청구를 기각했지."
○ 무기체: "기각. 당신 이혼 청구 기각이야."
```

### ④ 금액은 **장부(`ledger`)에 있는 값만** 쓴다

화마다 숫자가 달라지면 시청자가 계산을 못 한다. 판결문에서 뽑은 값을 한 벌
정해 놓고 그것만 쓴다. 장부 밖 금액이 대사에 나오면 반려한다.

### ⑤ 화마다 **때(`when`)** 를 적는다

이 사건은 여러 해에 걸쳐 있다. 화면 자막으로 첫 컷 앞 3초에 띄운다.
때가 거꾸로 가면 반려한다.

```
"when": "2013년 8월"   /   "when": "2017년 여름"
```

### ⑥ 주인공이 **없는 화**는 `irony: true` 로 표시한다

가해자 둘만 나오는 화(재산을 빼돌리는 장면 등)는 **시청자만 먼저 아는 화**다.
이게 숏드라마에서 가장 센 장치다 — 시청자가 주인공보다 먼저 알면 "빨리
알아채!" 하는 조바심이 생긴다. 표시해 두면 누설 검사에서 빼 준다.

### ⑦ 세 컷의 `SHOT` 크기를 **다 다르게**

1컷 두 사람(two-shot) → 2컷 어깨 너머(over-the-shoulder) → 3컷 얼굴(close-up).
...
- [ ] 화마다 `reveal`(새로 밝혀지는 것)·`must`(검사 낱말)·`when`(때)이 있는가
- [ ] 그 폭로가 **앞 화에 미리 새지 않는가** (주인공 없는 화는 `irony: true`)
- [ ] 마지막 대사가 **질문**으로 끝나는가 (마지막 화만 예외)
- [ ] 대사에 나오는 금액이 전부 `ledger` 에 있는 값인가
- [ ] 16화의 `reveal` 이 **서로 다른가** (같은 사실을 두 화가 나눠 갖지 않는가)
