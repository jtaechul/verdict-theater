...
// ⚠️⚠️⚠️ 2026-09-17 — 대본 이름과 카드 이름이 다른 사람이 하나 있다.
//    대본은 '아내' 라고 적는데 그림 파일·만들기(short90.ST_NAME)·받는 쪽
//    (fetch_cards.CARD_NAME)은 모두 '본처' 를 쓴다.
//    화면(아래 castOf)은 이 표로 맞춰 '본처' 라는 이름으로 얼굴을 올리는데,
//    **서버(castOfDoc)만 이 표를 안 보고** 대본 그대로 '아내' 만 인정했다.
//    → 아내 칸에만 "누구 그림인지 알 수 없습니다" 가 떴다 (손님 2026-09-17).
//      딸·내연녀 들은 표에 없어 이름이 그대로라 멀쩡히 올라갔다.
//    표를 보는 자리가 넷이다(화면·서버·만들기·받는 쪽). 하나라도 빠지면
//    올린 얼굴이 조용히 무시되거나 이렇게 거절당한다 — cast_check ② 가 견준다.
...
//    ⚠️ 셈한 뒤에는 **카드 이름으로 맞춘다**(아내 → 본처). 화면이 그 이름으로
//       올리고, 만들 때도 그 이름으로 찾는다.
function castOfDoc(doc) {
  const cast = new Set();
  for (const c of ((doc && doc.cuts) || []))
    for (const w of (c.who || [])) cast.add(CARD_NAME[String(w)] || String(w));
...
function partsCard(w) {
  ...
  //    까닭은 "한꺼번에" 가 아니라 **1편만 예약 없이 즉시 공개**되던 것이었다.
  ...
  h += '<div class="card"><h2>한 번에 올리기 (예약 공개)</h2>'
     ...
     + '공개돼 초기 노출이 나빠집니다. 실제로 1편만 그렇게 올라가 349회·0회'
     + '였고, 예약 공개한 편들은 1,212~2,946회였습니다.</div>'
